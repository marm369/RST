...
export function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsScrolled(scrollTop > 100); // Change après 100px de scroll
    };
  ...
  return (
    <header className={`fixed box-border h-[80px] w-full z-[100] left-0 top-0 md:h-[125px] transition-all duration-300 ${
      isScrolled 
      ...
      >
        ...
        >
          <span className="relative box-border block h-[18px] min-h-[auto] min-w-[auto] w-7 md:inline-block md:min-h-0 md:min-w-0">
            <span className={`absolute box-border block h-[3px] mt-[-1.5px] w-7 rounded-[3px] top-2/4 transition-all duration-300 ${
              isScrolled ? 'bg-emerald-900' : 'bg-white'
            } before:accent-auto before:box-border before:text-lg before:not-italic before:normal-nums before:font-normal before:h-[3px] before:tracking-[normal] before:leading-[27px] before:list-outside before:list-disc before:absolute before:text-center before:indent-[0px] before:normal-case before:top-[-7px] before:visible before:w-[calc(100%_-_5px)] before:rounded-[3px] before:border-separate before:font-forma_djr_display after:accent-auto after:bottom-[-7px] after:box-border after:text-lg after:not-italic after:normal-nums after:font-normal after:h-[3px] after:tracking-[normal] after:leading-[27px] after:list-outside after:list-disc after:absolute after:text-center after:indent-[0px] after:normal-case after:visible after:w-[calc(100%_-_10px)] after:rounded-[3px] after:border-separate after:left-[5px] after:font-forma_djr_display ${
              isScrolled 
                ? 'before:bg-emerald-900 after:bg-emerald-900' 
                : 'before:bg-white after:bg-white'
            }`}></span>
          </span>
        </button>

        <div className={`fixed [align-items:normal] box-border gap-x-5 flex flex-col h-[930px] justify-normal max-h-[930px] min-h-0 min-w-0 opacity-0 overscroll-x-contain overscroll-y-contain gap-y-5 translate-x-[25%] invisible w-full overflow-auto px-5 py-[35px] left-0 top-[80px] transition-all duration-300 ${
          ...
          <div className="box-border flex justify-center invisible md:visible">
            <a
              ...
              className={`text-base md:text-lg font-medium items-center box-border gap-x-0 flex h-auto justify-center leading-5 max-w-[300px] min-h-[65px] outline-transparent outline gap-y-0 text-center uppercase align-middle invisible w-full px-[53px] py-5 rounded-[35px] border-2 border-solid font-forma_djr_display transition-all duration-300 md:h-[75px] md:max-w-none md:min-h-[auto] md:visible md:w-auto md:px-[18px] md:py-3 md:rounded-[40px] ${
                isScrolled
